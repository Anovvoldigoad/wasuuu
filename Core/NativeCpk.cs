...
public static class NativeCpk
{
    ...
    private static extern int PackNative(string inputFolder, string outputCpk, int compress, int mode);

    [DllImport("cpkbridge", EntryPoint = "nsc_cpk_extract", CallingConvention = CallingConvention.Cdecl)]
    private static extern int ExtractNative(string inputCpk, string outputFolder);
