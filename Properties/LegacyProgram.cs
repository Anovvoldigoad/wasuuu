...
{
    /// <summary>
    /// Managed INI reader/writer used by the compiler and mod installer.
    /// Replaces kernel32 Get/WritePrivateProfileString so INI access behaves
    /// consistently under Wine/Winlator and on Windows.
    /// </summary>
    public class IniFile
    {
        private readonly string _path;
        private readonly string _defaultSection = Assembly.GetExecutingAssembly().GetName().Name ?? "NSC_ModManager";
        ...
        private Dictionary<string, string> EnsureSection(string section)
        {
            ...
            if (!_data.TryGetValue(section, out var values))
            {
        ...
        private void Save()
        {
            lock (_sync)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(section.Key)) sb.AppendLine("[" + section.Key + "]");
        ...
        public string Read(string Key, string Section = null)
        {
            lock (_sync)
            {
                string section = Section ?? _defaultSection;
                return _data.TryGetValue(section, out var values) &&
                       values.TryGetValue(Key ?? string.Empty, out var value)
                    ? value
                    : string.Empty;
            }
        }
        ...
        public void Write(string Key, string Value, string Section = null)
        {
            lock (_sync)
            {
                ...
                {
                    if (_data.TryGetValue(section, out var values)) values.Remove(Key);
                }
                else
                {
                    EnsureSection(section)[Key] = Value;
        ...
        public void DeleteKey(string Key, string Section = null) => Write(Key, null, Section ?? _defaultSection);
        public void DeleteSection(string Section = null) => Write(null, null, Section ?? _defaultSection);
